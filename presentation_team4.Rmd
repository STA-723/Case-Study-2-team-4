...
- Delete: id, host\_name and last\_review; 11 listings with price 0.
- Impute: impute 0's for `reviews\_per\_month` (10052 records).
- Categorize: minimum\_nights to 5 groups: (1,3], (3,7], (7,14], (14,21], (21,30], (30,$\infty$) days. 
- Transformation: log(price), log(1+reviews\_per\_month). 
- Incorporate new dataset: 
  - shape file for neighbourhoods (NYC Opendata)
    - relocate neighbourhoods
    - Obtain adjacency matrix for neighhoods
  - locations for metro stations
    - Obtain logarithm distance to the closest metro station for each listing
- Text cleaning:
  - Remove punctuations, stopwords, etc.
  - Word nomalization 
    - Apply Porter's stemmer algorithm to keep the root
    - e.g. luxury, luxurious $\rightarrow$ luxuri
