...
dat$minimum_nights[dat$minimum_nights>31] = 31
...
s =S.CARmultilevel(formula=log(price) ~ room_type + neighbourhood_group + log(1+reviews_per_month) + availability_365,
                family = 'gaussian', data = dat, ind.area=dat$ind_area,
                W=adjj, burnin=500, n.sample=1000)

s2 =S.CARmultilevel(formula=log(price) ~ room_type + log(1+reviews_per_month) + availability_365,family = 'gaussian', data = dat, ind.area=dat$ind_area,W=adjj, burnin=200, n.sample=500)

s3 = S.CARmultilevel(formula = log(1+reviews_per_month) ~ room_type + log(price) + availability_365,family = 'gaussian', data = dat, ind.area=dat$ind_area,W=adjj, burnin=200, n.sample=500)
